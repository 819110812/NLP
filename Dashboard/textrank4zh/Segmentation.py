...
def get_default_stop_words_file():
    ...
    return os.path.join(d, 'stopwords-en.txt')
...
class WordSegmentation(object):
    ...
    def segment(self, text, lower = True, use_stop_words = True, use_speech_tags_filter = False):
        ...
        if use_stop_words:
            nltk_result = [w for w in nltk_result if w not in self.stop_words]

        if use_speech_tags_filter:
            nltk_result = pos_tag(nltk_result)
        ...
        word_list = [w[0].strip() for w in nltk_result if len(w) > 1]
        # if lower:
